...
class categoriaProdutosController{
    ...
    index(req,res){
        ...
        db.query('SELECT * FROM categoria_produto ORDER BY id',(err,result)=>{
          if(err){
            console.log(`Houve um erro ao listar os clientes: ${err}`)
          }
          res.render('categoria-produto/listar',{categoria_produto:result.rows})
...
edit(req,res){
    ...
    db.query(query,(err,result)=>{
        ...
        res.render('categoria-produto/editar',{categoria_produto:result.rows[0]})
    })
}
...
module.exports = new categoriaProdutosController()